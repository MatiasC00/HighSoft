class Cliente:
    ...
    def __init__(self, numero, nombre, apellido, dni):
        ...
        if not isinstance(numero, int) or numero < 0:
            raise ValueError("El número de cliente debe ser INT y no puede ser negativo.")
        if not isinstance(nombre, str) or nombre == "":
            raise TypeError("El nombre de cliente debe ser STR y no debe estar vacío.")
        if not isinstance(apellido, str) or apellido == "":
            raise TypeError("El apellido de cliente debe ser STR y no debe estar vacío.")
        if not isinstance(dni, int) or dni < 0:
            raise ValueError("El DNI de cliente debe ser INT y no puede ser negativo.")
...
class ExcepcionLimite(Exception):
